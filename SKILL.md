...
9. Compose deterministically: `python scripts/slice_and_compose.py --mode compose --workdir work/ --output <final>.png`. The script pastes the rendered zones back at fixed coordinates through their zone masks and always keeps the Reality Anchor composited from the source. For Scheme B, inpaint the full canvas with `work/masks/zone{i}.png` and run `--mode enforce-anchor` instead. The four visible modules are the irregular contour-aware regions defined by the script's masks (default), supplied masks, or rect strips; they tile the canvas exactly with no gaps or overlaps. Keep them roughly balanced and readable, and preserve the Skill's modular color and palette rules.
