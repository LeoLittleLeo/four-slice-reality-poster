...
- Color abstraction increases with structural abstraction (Level-Dependent Color Reinterpretation): Reality stays photographic (source-faithful + uniform warm grade, never posterized), the 30% slice is a source-near palette interpretation, the 65% slice groups the shared palette into broader masses, and the 90% slice compresses to a small dominant subset (2–5 shared colors) while retaining shared color DNA and staying inside the shared poster palette.
