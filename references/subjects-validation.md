...
- The four regions share ONE shared limited palette (Robot Dreams Shared Palette System); color differences emerge from each region's abstraction rebalancing the proportions of the shared source colors — never from arbitrary module-level recoloring or per-region palette identities.
