...
- Color abstraction increases with structural abstraction (Level-Dependent Color Reinterpretation): Reality stays photographic (source-faithful + uniform warm grade, never posterized) → 30% is a source-near palette interpretation → 65% groups the shared palette into designed color masses → 90% compresses to a small dominant subset (2–5 shared colors) as symbolic shared-palette construction, while every step stays inside the shared poster palette and retains enough semantic color logic to stay traceable.
